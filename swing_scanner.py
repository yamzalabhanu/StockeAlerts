...
def _volume_score(direction, tech):
    avg_vol = safe_float(tech.get("avg_20_volume"))
    cur_vol = safe_float(tech.get("current_volume"))
    if not avg_vol or not cur_vol:
        return 0, []
    rel = cur_vol / avg_vol if avg_vol else 0
    score, reasons = 0, []
    if rel >= 3:
        score += 20; reasons.append(f"institutional volume {rel:.1f}x")
    elif rel >= 2:
        score += 15; reasons.append(f"strong volume {rel:.1f}x")
    elif rel >= 1.5:
        score += 10; reasons.append(f"volume confirmation {rel:.1f}x")
    elif rel < 0.7:
        score -= 8; reasons.append(f"weak volume {rel:.1f}x")
    return score, reasons
...
def _adx_proxy_score(direction, tech, price, ema20, ema50):
    """Use real ADX if supplied, otherwise approximate trend strength from EMA spread."""
    adx = tech.get("adx")
    score, reasons = 0, []
    if adx is not None:
        adx = safe_float(adx)
...
def score_swing_setup(tech: Dict) -> Optional[Dict]:
    """Professional swing confluence scorer.

    Uses trend + momentum + volume + MACD + ADX/trend-strength +
    support/resistance + relative strength + ATR risk management.
    Designed for 2-10 day holds.
    """
    if not tech:
        return None
    ...
    # Require at least 5 meaningful reasons so no single indicator can trigger swing trades.
    ...
    return {
        ...
        "timeframe": "Daily setup / 1H-4H entry timing",
        "reasons": reasons[:10],
